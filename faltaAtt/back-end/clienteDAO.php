    ...
    class ClienteDAO {
        ...
        public function listar() {
            ...
            while($cliente = pg_fetch_assoc($res)) {
                ...
                <td class="td">
                <a href="buscar.php?codcliente='.$cliente["codcliente"].'"><input name"editar" class="btn btn-outline-info" type="submit" value="Editar"></a>
        ...
        public function AtualizarCliente($cli, $codigo) {
            $MeuBD = $this->ConectarAoBanco();
            $sql = 'update cliente set nome = $1, cpf = $2, email = $3 WHERE codcliente = $4';
            pg_query_params($MeuBD, $sql, array($cli->ObterNome(), $cli->ObterCpf(), $cli->ObterEmail(), $cli->ObterCodigo())); 
            pg_close($MeuBD);
        }
    }
?>